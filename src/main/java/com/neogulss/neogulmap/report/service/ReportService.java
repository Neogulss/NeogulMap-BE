...
public class ReportService {
    ...
    public ReportDTO.SalesReportResponse getSalesReport(ReportDTO.Request request) {
        ...
        // 데이터가 null로 넘어오는 상황 구분
        ReportDTO.PreQuarterData preQuarterData = null;
        String preQuarterDataStatus = "NO_DATA";

        try {
            preQuarterData = reportMapper.selectPreQuarterData(request);
            if (preQuarterData != null) {
                preQuarterDataStatus = "OK";
            }
        } catch (Exception e) {
            log.error("[{}] REPORT_DATA_SALES 비교 데이터 조회 실패", request.getAdminDongCode(), e);
        ...
        return ReportDTO.SalesReportResponse.builder()
                ...
                // 추가
                .salesChangeRate(preQuarterData != null ? preQuarterData.getSalesChangeRate() : null)
                .salesToIndustryAvgRatio(preQuarterData != null ? preQuarterData.getSalesToIndustryAvgRatio() : null)
                .salesPerStore(preQuarterData != null ? preQuarterData.getSalesPerStore() : null)
                .preQuarterDataStatus(preQuarterDataStatus)
                .build();
    }
